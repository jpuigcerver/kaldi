...
#ifndef KALDI_DECODER_LATTICE_FASTER_INTERPLM_DECODER_H_
#define KALDI_DECODER_LATTICE_FASTER_INTERPLM_DECODER_H_
...
struct LatticeFasterInterpLmDecoderConfig {
  BaseFloat alpha;          // Linear interpolation weight
  BaseFloat beam;
  int32 max_active;
  int32 min_active;
  ...
  void Check() const {
    KALDI_ASSERT(alpha >= 0.0 && alpha <= 1.0 && beam > 0.0 && max_active > 1 &&
                 lattice_beam > 0.0 && prune_interval > 0 && beam_delta > 0.0 &&
 ...
 */
class LatticeFasterInterpLmDecoder {
 public:
  ...
  const LatticeFasterInterpLmDecoderConfig &GetOptions() const {
    return config_;
  }
 ...
 private:
  // Struct used to represent a state in the combined
  class StateTriplet {
   private:
    StateId state[3];
  ...
  struct ForwardLink {
    ...
    BaseFloat aco_cost;       // acoustic cost (pre-scaled) of traversing link
    BaseFloat hcl_cost;       // HCL cost of traversing link
    BaseFloat lm1_cost;       // LM1 cost of traversing link
    BaseFloat lm2_cost;       // LM2 cost of traversing link
